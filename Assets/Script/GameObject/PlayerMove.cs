...
public class PlayerMove : MonoBehaviour {
	...
	Rigidbody2D rb;
	public float angle;
	public Vector3 beforeVector;
	// Use this for  initialization
	void Start () {
		rb = gameObject.GetComponent<Rigidbody2D> ();
		beforeVector = Vector3.up;
	}
	...
	*/
	public void Move(Vector3 vector){
		...
		angle = Vector3.SignedAngle (beforeVector, this.transform.position, Vector3.forward);
		Debug.Log (angle);
		this.transform.rotation.SetFromToRotation (beforeVector, vector);
		beforeVector = this.transform.position;
	}
}
